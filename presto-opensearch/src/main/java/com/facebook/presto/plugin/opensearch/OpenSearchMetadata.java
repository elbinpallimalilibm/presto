        ...
        implements ConnectorMetadata
{
    ...
    @Inject
    public OpenSearchMetadata(OpenSearchClient client, OpenSearchConfig config)
    {
    ...
    public Map<String, ColumnHandle> getColumnHandles(ConnectorSession session, ConnectorTableHandle tableHandle)
    {
        ...
        Map<String, ColumnHandle> columns = new java.util.LinkedHashMap<>();
        ...
        if (config.isNestedFieldAccessEnabled()) {
            ...
            NestedFieldMapper nestedMapper = new NestedFieldMapper(config.getNestedMaxDepth());
            Map<String, NestedFieldInfo> allFields = nestedMapper.discoverNestedFields(mapping);

            // Add ALL fields as columns (both parent objects and leaf fields)
            // Parent objects will be exposed as VARCHAR (JSON strings)
            ...
            for (Map.Entry<String, NestedFieldInfo> entry : allFields.entrySet()) {
                ...
                Type columnType;
                if (fieldInfo.isLeafField()) {
                    // Leaf fields use their proper Presto type
                    columnType = fieldInfo.getPrestoType();
                }
                else {
                    // Parent objects (object/nested types) are exposed as VARCHAR (JSON strings)
                    columnType = VARCHAR;
                }
    ...
    private List<ColumnMetadata> getColumnsFromMapping(String indexName)
    {
        ...
        if (config.isNestedFieldAccessEnabled()) {
            ...
            if (!columnOrder.isEmpty()) {
                ...
                for (Map.Entry<String, NestedFieldInfo> entry : allFields.entrySet()) {
                    NestedFieldInfo fieldInfo = entry.getValue();
                    Type columnType = fieldInfo.isLeafField() ? fieldInfo.getPrestoType() : VARCHAR;
                    columns.add(ColumnMetadata.builder()
                            .setName(fieldInfo.getFieldPath())
                            .setType(columnType)
                            .setComment("")
